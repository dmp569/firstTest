/* Copyright 2006-2009 e-Learning Consulting. All Rights Reserved. See license agreement */
/* Version 1.04 */
/**
*
* Global Variables
...
function showLaunch() {
    ...
    sData += '<input type=text size=40 name="url" id="url" value="' + sMsgLaunch + '"  onkeypress="return noenter()" /> &nbsp;';
...
function restoreBuff(aState) {
    ...
    if (bRestore) {
        ...
        for (var i = 2; i < aState.length; i = i + 2) {
            /* get the name */
            sName = aState[i];
...
function launchIt() {
    /* get the file name */
    var sName = window.launch.document.getElementById("url").value;
    ...
    sLaunchParams += "," + getWindowSizeParams();
    //alert(sLaunchParams);
...
**/
function checkSCO() {
    /* A patch to IE causes an intermittent bug when we check for a closed window */
    try {
        ...
        /* we hit the IE error, so no more reason to check - cancel the interval timer */
        clearInterval(intervalId);
        intervalId = null;
...
function editData(sName) {
    ...
    var sData = '<form name="updateform" id="updateform">';
...
function updateData(sName) {
    ...
    var sValue = window.modify.document.getElementById("updateform").value;
